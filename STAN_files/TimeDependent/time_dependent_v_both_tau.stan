...
functions {
 matrix constructTransitionMatrix(real p_icoh, real p_coh){
  ...
  matrix odeUpdateMatrix(real[] th, int t, real dt){
    ...
    M[1,1] = -th[3]- th[2];
    M[1,2] = th[3];
    M[1,3] = -th[5]*exp(th[11]*(t-1)*dt);
    M[1,4] = -th[5]*exp(th[11]*(t-1)*dt);
    M[1,5] = -th[4]*exp(th[10]*(t-1)*dt);
    M[1,6] = -th[4]*exp(th[10]*(t-1)*dt);
    M[2,1] = th[3];
    M[2,2] = -th[3] - th[2];
    M[2,3] = th[5]*exp(th[11]*(t-1)*dt);
    M[2,4] = th[4]*exp(th[10]*(t-1)*dt);
    M[2,5] = th[5]*exp(th[11]*(t-1)*dt);
    M[2,6] = th[4]*exp(th[10]*(t-1)*dt);
    return M;
  }
...
transformed parameters{
  real theta[11]; //for metaphase ODE model
...
for (t in (T1+1):Frames) {
  ...
  for (t in (T0+1):T1) {
    if(t==(T0+1)) {
      for (j in 1:nStates){
        eta[t,j] = exp(normal_lpdf(y[t]| x0, sqrt(dt/(theta[1]*exp(theta[9]*dt*(t-2))))));
      }
    } else {
        ...
        eta[t,j] = exp(normal_lpdf(y[t]' | y[t-1]' + dt*odeUpdateMatrix(theta, t-1, dt)*auxStates + dt*odeUpdateVector(theta, cos_theta[t]), sqrt(dt/(theta[1]*exp(theta[9]*dt*(t-2))))));
      }
    }
  }
}
...
//    tau ~ gamma(0.5,1.0/1000);
  target+= gamma_lpdf(tau_0|0.5,10^(-3));
...
//  L ~ normal(0.790,0.119) T[0,];
  ...
  target+= normal_lpdf(tau_1|0,  log(10)/600);
