...
//is this helpful? https://brendanhasz.github.io/2018/11/15/hmm-vs-gp-part2.html
...
//Missing data procedure : https://mc-stan.org/docs/2_20/stan-users-guide/missing-data.html
data {
...
model {
  ...
  target+=normal_lpdf(v_d_plus|0,0.1)-log_diff_exp(normal_lcdf(2*v_plus_mean|0,0.1),normal_lcdf(-2*v_plus_mean|0,0.1));
    ...
    //Truncated normal !! In the case that the distributions are truncated the user needs 
    // to correct the renormallization see p.15 
  ...
  //   tau[iSis] ~ gamma(0.5,1.0/1000);
   target+=gamma_lpdf(tau[iSis]|0.5,10^(-3));
