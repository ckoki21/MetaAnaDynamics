...
functions {
 matrix constructTransitionMatrix(real p_icoh, real p_coh){
  ...
  matrix odeUpdateMatrix(real[] th, int t, real dt){
    // t is the index of frame
    // The actual time point at t^th frame is (t-1)*dt
    matrix[2,6] M;
    M[1,1] = -th[3]- th[2]*exp(th[9]*(t-1)*dt);
    M[1,2] = th[3];
    M[1,3] = -th[5];
    M[1,4] = -th[5];
    M[1,5] = -th[4];
    M[1,6] = -th[4];
    M[2,1] = th[3];
    M[2,2] = -th[3] - th[2]*exp(th[9]*(t-1)*dt);
    M[2,3] = th[5];
    M[2,4] = th[4];
    M[2,5] = th[5];
    M[2,6] = th[4];
...
//   alpha_0 ~ normal(0.01,100) T[0,];
  target+= normal_lpdf(alpha_0|0.01,0.1)-normal_lccdf(0|0.01,0.1); //T[0,] DO I NEED THIS?
...
//  alpha_1 ~ normal(0, log(10)/600)
//  the standard deviation here statisfies e^(log(10)/600 * 600) = 10
//  one standard deviation causes 10 fold change if metaphase is 600s
  target+= normal_lpdf(alpha_1|0, log(10)/600);
  target += sum(log(f));
}
