...
functions {
 matrix constructTransitionMatrix(real p_icoh, real p_coh){
  ...
  matrix odeUpdateMatrix(real[] th, int t, real dt){
    matrix[2,6] M;
    M[1,1] = -th[3]*exp(th[9]*(t-1)*dt)- th[2];
    M[1,2] = th[3]*exp(th[9]*(t-1)*dt);
    M[1,3] = -th[5];
    M[1,4] = -th[5];
    M[1,5] = -th[4];
    M[1,6] = -th[4];
    M[2,1] = th[3]*exp(th[9]*(t-1)*dt);
    M[2,2] = -th[3]*exp(th[9]*(t-1)*dt) - th[2];
    M[2,3] = th[5];
    M[2,4] = th[4];
    M[2,5] = th[5];
    M[2,6] = th[4];
  ...
  vector odeUpdateVector(real[] th, int t, real dt, real angleTheta){
    vector[2] mu;
    mu[1] = th[3]*exp(th[9]*(t-1)*dt)*th[8]*angleTheta;
    mu[2] = -th[3]*exp(th[9]*(t-1)*dt)*th[8]*angleTheta;
    return mu;
  }
...
transformed parameters{
  real theta[9]; //for metaphase ODE model
  ...
  for (t in (T0+1):T1) {
    if(t==(T0+1)) {
      for (j in 1:nStates){
        ...
        eta[t,j] = exp(normal_lpdf(y[t]' | y[t-1]' + dt*odeUpdateMatrix(theta, t-1, dt)*auxStates + dt*odeUpdateVector(theta, t-1, dt, cos_theta[t]), sqrt(dt/theta[1])));
      }
    }
  }
}
...
//   target+= normal_lpdf(kappa|0.05,0.1)-normal_lccdf(0|0.05,0.1);
  target+= normal_lpdf(kappa_0|0.05,0.1)-normal_lccdf(0|0.05,0.1); //is this better? Variances are markedtly different than the ones at Ar15
//   v_minus ~ normal(-0.03,0.1) T[,0];
  target+= normal_lpdf(v_minus|-0.03,0.1)-normal_lcdf(0|-0.03,0.1);
//   v_plus ~ normal(0.03,0.1) Tr[0,];
  target+=normal_lpdf(v_plus|0.03,0.1)-normal_lccdf(0|0.03,0.1);  
//  p_icoh ~ beta(2,1);
...
//  L ~ normal(0.790,0.119) T[0,];
  ...
  target+= normal_lpdf(kappa_1|0, log(10)/600);
  target += sum(log(f));
}
